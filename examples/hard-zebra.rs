...
fn lefto<U: UserState>(x: LTerm, y: LTerm, l: LTerm) -> Goal<U> {
...
fn rule1<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |c1, r1, c2, r2| {
...
// subscriber.
fn rule4<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        permuteo(
...
fn rule5<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |s1, s2| {
            [_, _, _, "5:00pm"] == s1,
            [_, "time", _, _] == s2,
...
fn rule6<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |r1, r2| {
...
fn rule7<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |r1, r2| {
...
fn rule8<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |r| {
...
fn rule9<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |r1, r2| {
...
fn rule10<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(
        |s1, s2| {
            [_, "fortune", _, _] == s1,
            ["jamari", _, _, _] == s2,
...
fn rule11<U: UserState>(answers: LTerm) -> Goal<U> {
    proto_vulcan!(membero([_, _, "mozzarella", "5:00pm"], answers))
}

fn hard_zebrao<U: UserState>(answers: LTerm) -> Goal<U> {
